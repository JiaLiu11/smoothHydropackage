...
def translate_centrality_cut(centrality_bound, cut_type='total_entropy'):
    ...
    try:
        centrality_cut_file = loadtxt(
            path.join(rootDir, 'centrality_cut_tables',centrality_cut_file_name))
    except IOError:
...
def generateAvgprofile(output_path, centrality_bounds, 
    ...
    runRecord = open(path.join(rootDir, 'runRecord.dat'), 'a')
    errRecord = open(path.join(rootDir, 'errRecord.dat'), 'a')
    ...
    superMC_folder = path.join(rootDir, 'superMC')
    # make result folder
    ...
    shutil.move(path.join(rootDir,'runRecord.dat'), 
        path.join(store_folder, 'runRecord.dat'))
    shutil.move(path.join(rootDir,'errRecord.dat'), 
        path.join(store_folder, 'errRecord.dat'))
...
if __name__ == "__main__":
    ...
    cut_type = 'total_entropy'
    collsys = 'Pb+Pb'.split('+')
    output_path = path.join(rootDir, 'RESULTS/initial_conditions')
    pre_eq = False
    ...
    while len(sys.argv) > 1:
        ...
        if option == '-model':
            ...
            output_path = path.join(rootDir, folder)
            del sys.argv[1]
        elif option == '-pre_eq':
