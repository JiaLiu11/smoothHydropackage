...
chiSq_criteria = 50 # 5 DOF: v2, v3, pion <pT>, proton <pT>, kaon <pT>
taus_bd = [0.1, 3]
etas_bd = [0,  0.21]
tdec_bd = [130, 170]
visbulknorm_bd = [0,4]
...
tdec_selected_array = data_selected[:, 2]*1000
...
# plot taus vs. tdec
plotScatter(taus_selected_array, tdec_selected_array, chisq_weight, 
    taus_bd+tdec_bd, axList[0,1])
axList[0,1].set_xlabel(r'$\tau_s$ (fm)', fontsize=plotfontsize+8)
axList[0,1].set_ylabel(r'$T_{sw}$ (MeV)', fontsize=plotfontsize+8)
# axList[1,0].set_yticks(np.linspace(0, 0.25, 6))
# axList[1,0].set_yticklabels(['%g'%i for i in np.linspace(0, 0.25, 6)])

# plot eta/s vs. tdec
plotScatter(etas_selected_array, tdec_selected_array, chisq_weight, 
    etas_bd+tdec_bd, axList[1,0])
axList[1,0].set_xlabel(r'$\eta/s$', fontsize=plotfontsize+8)
axList[1,0].set_ylabel(r'$T_{sw} (MeV)$', fontsize=plotfontsize+8)
#axList[1,0].set_yticks(np.array([100, 120, 140, 160]))
# axList[1,0].set_xticks(np.linspace(0, 0.25, 6))
# axList[1,0].set_xticklabels(['%g'%i for i in np.linspace(0, 0.25, 6)])

# plot bulk norm  vs. tdec
plotScatter(visbulknorm_selected_array, tdec_selected_array, chisq_weight, 
    visbulknorm_bd+tdec_bd, axList[1,1])
axList[1,1].set_xlabel(r'bulk norm', fontsize=plotfontsize+8)
axList[1,1].set_ylabel(r'$T_{sw}$ (MeV)', fontsize=plotfontsize+8)
for label in axList[1,1].get_xticklabels()[1::2]:
...
plt.savefig('chiSq_analysis_%s_4params_tsw.pdf'%init_model)
plt.show()