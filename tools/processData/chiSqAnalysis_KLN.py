...
# run constant
init_model = "hasfs_kln" # kln, kln
inputFile_name = "%s_tet_chiSq.dat"%init_model
...
chisq_bd= [0, 850]
exp_factor = 2e5
exp_factor=1e5 if(init_model=="hasfs_kln") else 40000
init_model_label = "kln w/ pre-eq." if(init_model=="hasfs_kln") else "kln w/o pre-eq."
...
plt.savefig('chiSq_analysis_%s.pdf'%init_model)
plt.show()